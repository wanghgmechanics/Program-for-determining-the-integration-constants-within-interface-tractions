...
% as shown in Table 2 on pages 19-20 for stiffened plate joint.
...
A6 = [1/gamma4, 1/gamma5, 1/gamma6, 1/gamma7
    1/gamma4^3, 1/gamma5^3, 1/gamma6^3, 1/gamma7^3
...
A6 = [1/lambda1,1/lambda2,1/lambda3,1/lambda4,1/lambda5,1/lambda6
     ...
     1/lambda1^3,1/lambda2^3,1/lambda3^3,1/lambda4^3,1/lambda5^3,1/lambda6^3
...
B6 = [0
      0
      0
      -(barM0 - C10*barh2 + C13)/(1+barh2)
      0
      -C8/k4 + k2*(barM0 - C10*barh2 + C13)/(1+barh2)];
...
A12 = [1/Lambda1,1/Lambda2,1/Lambda3,1/Lambda4,1/Lambda5,1/Lambda6
     ...
     exp(Lambda1*bara)/Lambda1^2,  exp(Lambda2*bara)/Lambda2^2,  exp(Lambda3*bara)/Lambda3^2  ,exp(Lambda4*bara)/Lambda4^2,  exp(Lambda5*bara)/Lambda5^2,  exp(Lambda6*bara)/Lambda6^2
     Lambda1,Lambda2,Lambda3,Lambda4,Lambda5,Lambda6
     exp(Lambda1*bara)*(Lambda1^2+d3),  exp(Lambda2*bara)*(Lambda2^2+d3),  exp(Lambda3*bara)*(Lambda3^2+d3)  ,exp(Lambda4*bara)*(Lambda4^2+d3),  exp(Lambda5*bara)*(Lambda5^2+d3),  exp(Lambda6*bara)*(Lambda6^2+d3)];
B12 = [0
      0
      0
      -(barM0 - H10*barh2 + H13)/(1+barh2)
      0
      -d5*H8 + d4*(barM0 - H10*barh2 + H13)/(1+barh2)];
